...
export const generateChart = (data) => {
   ...
   const balansAmounts = incomeAmounts.map((income, i) => income - expensesAmounts[i]);
   ...
   myChart = new Chart(ctx, {
      ...
      data: {
         labels: chartLabel,
         datasets: [
            {label: "Dohodi", data: incomeAmounts, borderWidth: 2, hidden: true},
            {label: "Rasohodi", data: expensesAmounts, borderWidth: 2, hidden: true},
            {label: "Balans", data: balansAmounts, borderWidth: 2, hidden: false}
         ],
      },
      options: {
         scales: {
